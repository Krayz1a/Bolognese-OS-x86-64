...
ffffffff80000520:	48 8b 05 61 30 00 00 	mov    0x3061(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff80000540:	48 8b 05 41 30 00 00 	mov    0x3041(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff80000593:	e8 18 05 00 00       	call   ffffffff80000ab0 <print_hex>
                for (size_t idx = 0; idx < memmap_request.response->entry_count; idx++) {
ffffffff80000598:	48 8b 05 e9 2f 00 00 	mov    0x2fe9(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
uint64_t pmm_max_usable_addr(void) {
	...
	if(memmap_request.response != NULL) {
                for (size_t idx = 0; idx < memmap_request.response->entry_count; idx++) {
ffffffff800005c0:	48 8b 05 c1 2f 00 00 	mov    0x2fc1(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff800005d4:	0f 1f 40 00          	nopl   0x0(%rax)
                        struct limine_memmap_entry *mm = memmap_request.response->entries[idx];
ffffffff800005d8:	48 8b 15 a9 2f 00 00 	mov    0x2fa9(%rip),%rdx        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff800005f8:	48 0f 42 f2          	cmovb  %rdx,%rsi
                for (size_t idx = 0; idx < memmap_request.response->entry_count; idx++) {
ffffffff800005fc:	48 8b 15 85 2f 00 00 	mov    0x2f85(%rip),%rdx        # ffffffff80003588 <memmap_request+0x28>
...
uint64_t pmm_get_bitmap_pbase(void){
	if(memmap_request.response != NULL) {
                for (size_t idx = 0; idx < memmap_request.response->entry_count; idx++) {
ffffffff80000620:	48 8b 05 61 2f 00 00 	mov    0x2f61(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff80000630:	48 8b 0d f9 49 00 00 	mov    0x49f9(%rip),%rcx        # ffffffff80005030 <bitmap_size>
                for (size_t idx = 0; idx < memmap_request.response->entry_count; idx++) {
ffffffff80000637:	31 c0                	xor    %eax,%eax
ffffffff80000639:	0f 1f 80 00 00 00 00 	nopl   0x0(%rax)
                        struct limine_memmap_entry *mm = memmap_request.response->entries[idx];
ffffffff80000640:	48 8b 15 41 2f 00 00 	mov    0x2f41(%rip),%rdx        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff8000065a:	72 14                	jb     ffffffff80000670 <pmm_get_bitmap_pbase.part.0+0x50>
                for (size_t idx = 0; idx < memmap_request.response->entry_count; idx++) {
ffffffff8000065c:	48 8b 15 25 2f 00 00 	mov    0x2f25(%rip),%rdx        # ffffffff80003588 <memmap_request+0x28>
...
void pmm_zero_usable_pages(void) {
	if(memmap_request.response != NULL) {
		for (size_t idx = 0; idx < memmap_request.response->entry_count; idx++) {
ffffffff80000680:	48 8b 05 01 2f 00 00 	mov    0x2f01(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff8000069c:	0f 1f 40 00          	nopl   0x0(%rax)
		for (size_t idx = 0; idx < memmap_request.response->entry_count; idx++) {
ffffffff800006a0:	48 8b 05 e1 2e 00 00 	mov    0x2ee1(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff800006af:	73 6f                	jae    ffffffff80000720 <pmm_zero_usable_pages.part.0+0xa0>
			struct limine_memmap_entry *mm = memmap_request.response->entries[idx];
ffffffff800006b1:	48 8b 05 d0 2e 00 00 	mov    0x2ed0(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff800006f3:	48 c1 ea 0f          	shr    $0xf,%rdx
        bitmap[byte_idx] &= ~(1 << bit_idx);
ffffffff800006f7:	48 03 15 42 49 00 00 	add    0x4942(%rip),%rdx        # ffffffff80005040 <bitmap>
...
ffffffff80000720:	48 8b 05 11 49 00 00 	mov    0x4911(%rip),%rax        # ffffffff80005038 <bitmap_pbase>
ffffffff80000727:	48 8b 15 02 49 00 00 	mov    0x4902(%rip),%rdx        # ffffffff80005030 <bitmap_size>
...
ffffffff80000752:	48 c1 ea 0f          	shr    $0xf,%rdx
        bitmap[byte_idx] |= (1 << bit_idx);
ffffffff80000756:	48 03 15 e3 48 00 00 	add    0x48e3(%rip),%rdx        # ffffffff80005040 <bitmap>
        uint8_t bit_idx = frame_idx % 8;
...
ffffffff80000762:	40 08 3a             	or     %dil,(%rdx)
		for (uint64_t p = bitmap_pbase; p < bitmap_pbase + bitmap_size; p += PAGE_SIZE) {
ffffffff80000765:	48 8b 15 c4 48 00 00 	mov    0x48c4(%rip),%rdx        # ffffffff80005030 <bitmap_size>
ffffffff8000076c:	48 03 15 c5 48 00 00 	add    0x48c5(%rip),%rdx        # ffffffff80005038 <bitmap_pbase>
...
ffffffff8000078e:	48 c1 e8 0f          	shr    $0xf,%rax
        bitmap[byte_idx] |= (1 << bit_idx);
ffffffff80000792:	48 03 05 a7 48 00 00 	add    0x48a7(%rip),%rax        # ffffffff80005040 <bitmap>
        uint8_t bit_idx = frame_idx % 8;
...
ffffffff800007bf:	48 c1 e9 0c          	shr    $0xc,%rcx
        bitmap[byte_idx] &= ~(1 << bit_idx);
ffffffff800007c3:	48 03 05 76 48 00 00 	add    0x4876(%rip),%rax        # ffffffff80005040 <bitmap>
...
ffffffff800007d0 <pmm_dump_usable_memmap>:
	if(memmap_request.response != NULL) {
ffffffff800007d0:	48 8b 05 b1 2d 00 00 	mov    0x2db1(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff800007f0 <pmm_max_usable_addr>:
	if(memmap_request.response != NULL) {
ffffffff800007f0:	48 8b 05 91 2d 00 00 	mov    0x2d91(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff80000810 <pmm_get_bitmap_size>:
	return (((highest_usable_addr + PAGE_SIZE - 1) / PAGE_SIZE) + 7) / 8;
ffffffff80000810:	48 8b 05 11 48 00 00 	mov    0x4811(%rip),%rax        # ffffffff80005028 <highest_usable_addr>
...
ffffffff80000830 <pmm_get_bitmap_pbase>:
	if(memmap_request.response != NULL) {
ffffffff80000830:	48 8b 05 51 2d 00 00 	mov    0x2d51(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff80000850 <pmm_zero_usable_pages>:
	if(memmap_request.response != NULL) {
ffffffff80000850:	48 8b 05 31 2d 00 00 	mov    0x2d31(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff80000870:	4c 8b 05 b9 47 00 00 	mov    0x47b9(%rip),%r8        # ffffffff80005030 <bitmap_size>
	
	// Start scanning from where we last found a free page
	for (size_t i = last_scanned_i; i < bitmap64_size; i++) {
ffffffff80000877:	48 8b 15 ca 47 00 00 	mov    0x47ca(%rip),%rdx        # ffffffff80005048 <last_scanned_i>
	uint64_t* bitmap_64 = (uint64_t*)bitmap;
ffffffff8000087e:	48 8b 3d bb 47 00 00 	mov    0x47bb(%rip),%rdi        # ffffffff80005040 <bitmap>
	size_t bitmap64_size = bitmap_size / 8;
...
ffffffff800008ce:	42 08 14 07          	or     %dl,(%rdi,%r8,1)
                ...
                pmm_mark_used(paddr);
		last_scanned_i = i;
ffffffff800008d2:	48 89 35 6f 47 00 00 	mov    %rsi,0x476f(%rip)        # ffffffff80005048 <last_scanned_i>
                return paddr;
...
ffffffff80000983:	48 c1 e9 0c          	shr    $0xc,%rcx
        bitmap[byte_idx] &= ~(1 << bit_idx);
ffffffff80000987:	48 03 05 b2 46 00 00 	add    0x46b2(%rip),%rax        # ffffffff80005040 <bitmap>
ffffffff8000098e:	d2 c2                	rol    %cl,%dl
ffffffff80000990:	20 10                	and    %dl,(%rax)
	
	if (freed_chunk_idx < last_scanned_i) {
ffffffff80000992:	48 3b 3d af 46 00 00 	cmp    0x46af(%rip),%rdi        # ffffffff80005048 <last_scanned_i>
ffffffff80000999:	73 07                	jae    ffffffff800009a2 <pmm_free_page+0x32>
		last_scanned_i = freed_chunk_idx;
ffffffff8000099b:	48 89 3d a6 46 00 00 	mov    %rdi,0x46a6(%rip)        # ffffffff80005048 <last_scanned_i>
	}
}
...
ffffffff800009b0:	55                   	push   %rbp
	if(memmap_request.response != NULL) {
ffffffff800009b1:	48 8b 05 d0 2b 00 00 	mov    0x2bd0(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
void pmm_init(void){
...
ffffffff800009dc:	48 89 05 45 46 00 00 	mov    %rax,0x4645(%rip)        # ffffffff80005028 <highest_usable_addr>
	if(memmap_request.response != NULL) {
ffffffff800009e3:	48 8b 05 9e 2b 00 00 	mov    0x2b9e(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
	return 0;
ffffffff800009ea:	31 ff                	xor    %edi,%edi
	bitmap_size = pmm_get_bitmap_size();
ffffffff800009ec:	48 89 35 3d 46 00 00 	mov    %rsi,0x463d(%rip)        # ffffffff80005030 <bitmap_size>
	if(memmap_request.response != NULL) {
...
ffffffff80000a00:	48 8b 05 41 2b 00 00 	mov    0x2b41(%rip),%rax        # ffffffff80003548 <hhdm_request+0x28>
	bitmap_pbase = pmm_get_bitmap_pbase();
ffffffff80000a07:	48 89 3d 2a 46 00 00 	mov    %rdi,0x462a(%rip)        # ffffffff80005038 <bitmap_pbase>
...
ffffffff80000a1a:	48 89 3d 1f 46 00 00 	mov    %rdi,0x461f(%rip)        # ffffffff80005040 <bitmap>
	memset(bitmap, 0xFF, bitmap_size);
ffffffff80000a21:	e8 ea 00 00 00       	call   ffffffff80000b10 <memset>
	if(memmap_request.response != NULL) {
ffffffff80000a26:	48 8b 05 5b 2b 00 00 	mov    0x2b5b(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff80000a32:	e8 49 fc ff ff       	call   ffffffff80000680 <pmm_zero_usable_pages.part.0>
	if(memmap_request.response != NULL) {
ffffffff80000a37:	48 8b 05 4a 2b 00 00 	mov    0x2b4a(%rip),%rax        # ffffffff80003588 <memmap_request+0x28>
...
ffffffff80000a54:	48 8b 3d cd 45 00 00 	mov    0x45cd(%rip),%rdi        # ffffffff80005028 <highest_usable_addr>
...
ffffffff80000a6c:	48 8b 3d cd 45 00 00 	mov    0x45cd(%rip),%rdi        # ffffffff80005040 <bitmap>
